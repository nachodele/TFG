...
-  δ: Q   x Σ → Q : Función de transición que define cómo el autómata cambia de estado al procesar un símbolo.
-  q_0 ∈ Q : Estado inicial.
-  F ⊆ Q : Conjunto de estados finales.

La función de transición extendida  T^* : Q   x Σ^* → Q  se define como:
1. Caso base: Para la cadena vacía ( ε ):
    
   T^*(q, ε) = q
        Es decir, si no se procesa ningún símbolo, el autómata permanece en el estado actual.

2. Caso inductivo: Para una cadena  w = xa , donde  x ∈ Σ^*  y  a ∈ Σ :
