...
Dado un APV \( M = (Q, \Sigma, \Gamma, \delta, q_0, Z_0) \), el objetivo es construir una gramática \( G = (N, T, P, S) \) tal que:
\[
L(G) = L(M).
\]
...
1. **Símbolos No Terminales (\( N \))**:
   - Cada no terminal tiene la forma \( [q, A, p] \), donde:
     - \( q, p \in Q \): Son estados del APV.
     - \( A \in \Gamma \): Es un símbolo de la pila.
...
2. **Símbolos Terminales (\( T \))**:
   - Los símbolos terminales son los mismos que el alfabeto de entrada del APV (\( T = \Sigma \)).
