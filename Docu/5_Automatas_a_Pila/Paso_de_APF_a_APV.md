...
3. **Añadir transiciones para vaciar la pila**:
   - Crear transiciones-\( \epsilon \) desde los estados finales originales hacia un nuevo estado que se encargue de vaciar la pila.
...
Dado un APF \( M = (Q, \Sigma, \Gamma, \delta, q_0, Z_0, F) \), construimos un APV equivalente \( M' = (\bar{Q}, \Sigma, \bar{\Gamma}, \bar{\delta}, \bar{q}_0, \bar{Z}_0) \) siguiendo estos pasos:

1. **Definir los nuevos componentes**:
   - \( \bar{Q} = Q \cup \{q_v\} \), donde \( q_v \) es un nuevo estado encargado de vaciar la pila.
   - \( \bar{\Gamma} = \Gamma \cup \{Z'\} \), donde \( Z' \) es el nuevo símbolo inicial de la pila.
   - \( \bar{q}_0 = q_0 \), el estado inicial permanece igual.
   - \( \bar{Z}_0 = Z' \), el nuevo símbolo inicial de la pila.

2. **Modificar las transiciones**:
   - Al iniciar el autómata, apilar \( Z'Z_0 \):
     - \( \bar{\delta}(q_0, ε, Z') = (q_0, Z'Z_0) \).
   - Para cada estado final \( f \in F\), añadir una transición-\( ε\) hacia \( q_v\):
     - \( \bar{\delta}(f, ε, Z') = (q_v, ε) \).
   - En \( q_v\), añadir transiciones-\( ε\) para vaciar la pila:
     - Para cada símbolo \( X \in \Gamma\):
       - \( \bar{\delta}(q_v, ε, X) = (q_v, ε) \).
