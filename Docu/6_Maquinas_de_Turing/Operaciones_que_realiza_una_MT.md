...
### 2. **Escritura en la Cinta**
   - La MT puede sobrescribir el símbolo actual en la celda de la cinta con un nuevo símbolo del alfabeto de la cinta (\( \Gamma \)).
...
### 4. **Cambio de Estado**
   - La MT cambia su estado actual según lo definido por su función de transición \( \delta(q, X) = (q', Y, D) \).
   - Este cambio refleja el progreso del cómputo.

### 5. **Detención**
   - La MT se detiene si alcanza un estado final (\( q_f \in F \)) o si no hay una transición definida para el estado y símbolo actuales.
...
La función de transición \( \delta: Q \times \Gamma \to Q \times \Gamma \times \{L, R\} \) define las operaciones que realiza una MT:
1. \( Q \): Conjunto finito de estados.
2. \( \Gamma \): Alfabeto de la cinta (incluye el símbolo blanco \( B \)).
3. \( L, R \): Movimientos hacia la izquierda o derecha.

Para cada configuración \( (q, X) \), donde:
- \( q \in Q \): Estado actual.
- \( X \in \Gamma \): Símbolo leído en la celda actual.

La función \( \delta(q, X) = (q', Y, D) \) indica:
