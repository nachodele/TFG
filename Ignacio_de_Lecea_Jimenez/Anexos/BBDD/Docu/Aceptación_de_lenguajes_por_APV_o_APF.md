...
#### Definición Formal
Sea  M = (Q, Σ, Γ, δ, q_0, Z_0)  un AP. El lenguaje aceptado por vaciado de pila es:
 
L(M) =  w ∈ Σ^* : (q_0, w, Z_0) ⊢^* (q, ε, ε) ; q ∈ Q .
  Donde:
-  w : Cadena de entrada.
...
#### Definición Formal
Sea  M = (Q, Σ, Γ, δ, q_0, Z_0, F)  un AP. El lenguaje aceptado por estados finales es:
 
L(M) =  w ∈ Σ^* : (q_0, w, Z_0) ⊢^* (q_f, ε, Γ) ; q_f ∈ F ; Γ ∈ Γ^* .
  Donde:
