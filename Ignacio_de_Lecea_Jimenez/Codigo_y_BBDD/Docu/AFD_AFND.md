...
-  δ: Q   x Σ → Q : Función de transición que asigna un único estado a cada par (estado actual, símbolo de entrada).
-  q_0 ∈ Q : Estado inicial.
-  F ⊆ Q : Conjunto de estados finales o de aceptación.

### Propiedades
1. Determinismo:
   - Para cada estado  q ∈ Q  y cada símbolo  a ∈ Σ , existe exactamente una transición definida ( δ(q, a) = p ).
...
### Propiedades
1. No determinismo:
   - Para un estado  q ∈ Q  y un símbolo  a ∈ Σ, pueden existir varias transiciones posibles ( δ(q,a) ⊆ Q).
...
- Transiciones:
  ...
  -  δ(q_1, 0) = {q_2} , δ(q_1, 1) = ∅; δ(q_2,a)=∅,∀ a∈Σ.
