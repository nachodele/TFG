...
Formalmente:
\[
x \text{ es una forma sentencial de } G \iff S \overset{*}{\Rightarrow} x
\]
Donde \( S \) es el símbolo inicial, \( x \in (N \cup T)^* \), y \( \overset{*}{\Rightarrow} \) denota una relación de derivación en uno o más pasos.
...
Formalmente:
\[
x \text{ es una sentencia de } G \iff S \overset{*}{\Rightarrow} x \text{ y } x \in T^*
\]
...
- \( S = S \): Símbolo inicial.
- \( P = \{S \rightarrow aA, A \rightarrow bS, S \rightarrow ab, A \rightarrow b\}\): Reglas de producción.
...
1. **Definir lenguajes formales**: El conjunto de todas las sentencias generadas por una gramática constituye su lenguaje formal (\( L(G) = \{x : S \overset{*}{\Rightarrow} x, x \in T^*\} \)).
