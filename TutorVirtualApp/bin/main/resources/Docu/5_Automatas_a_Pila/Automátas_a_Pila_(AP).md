...
- \( \Gamma \): Alfabeto de la pila (símbolos que pueden almacenarse en la pila).
- \( \delta: Q \times (\Sigma \cup \{\epsilon\}) \times \Gamma \to P(Q \times \Gamma^*) \): Función de transición.
...
La función \( \delta(q, a, X) = (p, w) \) significa que:
1. Si el autómata está en el estado \( q \),
2. Lee el símbolo \( a \) de la entrada (o no consume nada si \( a = \epsilon\)),
