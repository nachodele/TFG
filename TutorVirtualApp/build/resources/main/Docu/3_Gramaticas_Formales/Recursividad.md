...
Ejemplo:
- Definición recursiva del conjunto de números pares (\( \text{EVEN} \)):
  1. \( 2 \in \text{EVEN} \) (caso base).
  2. Si \( x \in \text{EVEN} \), entonces \( x + 2 \in \text{EVEN} \) (regla recursiva).
  3. Ningún otro número pertenece a \( \text{EVEN} \) (condición de exhaustividad).
...
1. **Recursividad por la Izquierda**:
   ...
   - Ejemplo:
     ...
     \]
     Donde \( A \) es un símbolo no terminal, \( \alpha \) es una cadena (posiblemente vacía) y \( \beta \) es una cadena que no comienza con \( A \).
...
\]
Donde \( A\alpha_i \) son producciones recursivas y \( \beta_j \) son producciones no recursivas:
