...
4. **Gestionar producciones con cadena vacía (\(\epsilon\))**  
   - Si la gramática permite la derivación de la cadena vacía (\(\epsilon\)) y el no terminal que la genera es \( S \), se marcará el estado inicial como final.  
   - Si se generan vacíos desde no terminales distintos de \( S \), suele ser necesario un ajuste adicional (como crear transiciones \(\epsilon\) o eliminar esas reglas según la equivalencia que se desee).
...
> Nótese que, en una implementación determinista, la regla \(S \rightarrow 1\) suele manejarse con un estado adicional si se desea evitar la no determinación. El enfoque exacto depende de si se prefiere un AFND o un AFD.
...
3. **Gramáticas con \(\epsilon\)**: Si la gramática puede generar la cadena vacía (\(\epsilon\)), se debe marcar el estado inicial como final o manipular las reglas para reflejar este caso, según la construcción deseada.  
