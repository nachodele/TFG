...
1. **Definir el Estado Inicial del AFD**:
   - El estado inicial del AFD corresponde al conjunto de estados alcanzables desde el estado inicial del AFND mediante transiciones-\( \epsilon \) (si existen).

2. **Construir los Estados del AFD**:
   - Cada estado del AFD representa un subconjunto de estados del AFND.
   - Para cada símbolo \( a \in \Sigma \), calcular las transiciones desde cada subconjunto hacia otros subconjuntos.
