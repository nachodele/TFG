...
- Para cada estado \( q \in Q \) y cada símbolo \( a \in \Sigma \), existe exactamente una transición definida (\( \delta(q, a) \) es único).
...
Un **NFA** permite múltiples transiciones para un mismo estado y símbolo de entrada (\( |\delta(q, a)| > 1\)) o incluso ninguna transición.
