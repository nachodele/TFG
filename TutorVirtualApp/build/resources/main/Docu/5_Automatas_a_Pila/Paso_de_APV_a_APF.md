...
2. Crear transiciones-\( \epsilon \) desde las configuraciones donde la pila está vacía hacia este nuevo estado final.
3. Eliminar la dependencia del vaciado de la pila como criterio de aceptación.

### Pasos del Algoritmo

Dado un APV \( M = (Q, \Sigma, \Gamma, \delta, q_0, Z_0) \), construimos un APF equivalente \( M' = (\bar{Q}, \Sigma, \Gamma, \bar{\delta}, q_0, Z_0, F') \) siguiendo estos pasos:

1. **Definir los nuevos componentes**:
   - \( \bar{Q} = Q \cup \{q_f\} \), donde \( q_f \) es un nuevo estado final.
   - \( F' = \{q_f\} \), el conjunto de estados finales contiene únicamente \( q_f \).

2. **Añadir transiciones-\( \epsilon \)**:
   - Para cada estado \( q \in Q \), añadir una transición-\( \epsilon \) hacia \( q_f \) si la pila está vacía:
     - \( \bar{\delta}(q, ε, Z_0) = (q_f, Z_0) \).
