...
\[
\text{Dado } M \text{ (una Máquina de Turing) y } w \text{ (una entrada), decidir si } M(w) \text{ termina.}
\]
...
2. Construyamos una nueva Máquina de Turing \( D \) que utiliza \( H \) como subrutina:
   - Si \( H(M, M) = \text{"sí"} \), entonces \( D(M) \) entra en un bucle infinito.
   - Si \( H(M, M) = \text{"no"} \), entonces \( D(M) \) se detiene.

3. Ahora evaluemos el comportamiento de \( D(D) \):
   - Si \( H(D, D) = \text{"sí"} \), entonces \( D(D) \) entra en un bucle infinito, lo cual es contradictorio.
   - Si \( H(D, D) = \text{"no"} \), entonces \( D(D) \) se detiene, lo cual también es contradictorio.
...
\[
A = \{u_1, u_2, ..., u_k\}, \quad B = \{v_1, v_2, ..., v_k\},
\]
¿existe una secuencia finita de índices \( i_1, i_2, ..., i_n \), con \( 1 \leq i_j \leq k \), tal que las cadenas concatenadas sean iguales?
\[
...
Supongamos:
\[
A = \{u_1 = "a", u_2 = "ab"\}, \quad B = \{v_1 = "aa", v_2 = "b"\}.
\]
