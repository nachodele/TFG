...
2. **Hojas**: Las hojas del árbol corresponden a símbolos terminales o a la cadena vacía (\( \lambda \)).
...
3. Continuar hasta que todas las hojas sean símbolos terminales o la cadena vacía (\( \lambda \)).
...
2. **Teorema Fundamental**:
   - Sea \( G = (N, T, S, P) \) una gramática libre de contexto y sea \( w \in T^* \). Entonces:
     - \( w \in L(G) \) si y solo si existe un árbol de derivación en \( G \) cuya producción es \( w \).
