...
- \( A \rightarrow Ba \), donde \( A, B \in N \) y \( a \in T \),
...
1. **Invertir la Gramática**:
   - Dada una G3LI \( G = (N, T, P, S) \), invertir las producciones para obtener una gramática lineal por la derecha (G3LD):
     - Si \( A \rightarrow Ba \), se convierte en \( A' \rightarrow aB' \).
     - Si \( A \rightarrow a \), se convierte en \( A' \rightarrow a \).
...
2. **Construir el Autómata Finito No Determinista (\( ε-AFND \))**:
   ...
   - Para cada producción invertida:
     - Si \( A' \rightarrow aB' \), añadir una transición del estado correspondiente a \( A' \) hacia el estado correspondiente a \( B' \) con el símbolo \( a \).
     - Si \( A' \rightarrow a \), añadir una transición del estado correspondiente a \( A' \) hacia un estado final con el símbolo \( a \).
