...
- \( u \in \Sigma^* \): Es la parte de la cadena de entrada que queda por procesar.
- \( \gamma \in \Gamma^* \): Es el contenido actual de la pila, donde el primer símbolo de \( \gamma \) representa la cima de la pila.
...
- La pila \( \gamma \) refleja la memoria auxiliar del autómata en ese momento.
...
\[
(q, au, X\gamma) \vdash (p, u, w\gamma),
\]
donde:
...
\[
C_i \vdash C_{i+1} \vdash C_{i+2} \vdash ...,
\]
