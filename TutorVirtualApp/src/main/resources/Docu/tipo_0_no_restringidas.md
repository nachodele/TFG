...
-  P : Conjunto finito de reglas de producción de la forma:
  ...
  α  → β
      Donde:
  -  α, β ∈ (N ∪ T)^*  (cadenas formadas por terminales y no terminales).
  -  |α| > 0 , es decir,  α  no puede ser la cadena vacía ( ε ).

### Características Clave
- No existen restricciones sobre las reglas de producción más allá de que el lado izquierdo ( α ) no puede estar vacío.
