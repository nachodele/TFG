...
### Definición
Dos AFDs \( M_1 = (Q_1, \Sigma, \delta_1, q_0^1, F_1) \) y \( M_2 = (Q_2, \Sigma, \delta_2, q_0^2, F_2) \) son equivalentes si reconocen el mismo lenguaje. Es decir:
\[
L(M_1) = L(M_2)
\]
...
Dos estados \( p \in Q_1 \) y \( q \in Q_2 \) son equivalentes si:
\[
\forall x \in \Sigma^*,\; \delta^*_1(p, x) \in F_1 \iff \delta^*_2(q, x) \in F_2
\]
...
1. **Construcción del Producto Cruzado**:
   - Crear un nuevo autómata \( M = (Q, \Sigma, \delta, (q_0^1, q_0^2), F) \), donde:
     - \( Q = Q_1 \times Q_2 \),
     - \( F = (F_1 \times F_2) \cup ((Q_1 - F_1) \times (Q_2 - F_2)) \),
