...
\]
donde:
- \( A \in N \): Es un símbolo no terminal.
- \( a \in T \): Es un símbolo terminal.
- \( \alpha \in N^* \): Es una cadena (posiblemente vacía) de símbolos no terminales.
...
### Componentes del APV
Dada una gramática \( G = (N, T, P, S) \) en FNG, el APV asociado \( M = (Q, \Sigma, \Gamma, \delta, q_0, Z_0) \) se define como:
- \( Q = \{q_0, q_1\} \): Conjunto de estados.
  - \( q_0 \): Estado inicial.
  - \( q_1 \): Estado final.
- \( \Sigma = T \): Alfabeto de entrada (símbolos terminales).
- \( \Gamma = N \cup T \cup \{Z_0\} \): Alfabeto de la pila.
  - Incluye los no terminales (\( N \)), los terminales (\( T \)) y un símbolo inicial especial (\( Z_0 \)).
- \( Z_0 \in \Gamma \): Símbolo inicial de la pila.
- \( F = \emptyset \): No hay estados finales porque la aceptación es por vaciado de pila.

### Transiciones
La función de transición \( \delta(q, a, X) = (p, w) \) se define como:
1. **Inicialización**:
...
2. **Procesamiento de Producciones**:
   - Para cada producción \( A → a\alpha \):
