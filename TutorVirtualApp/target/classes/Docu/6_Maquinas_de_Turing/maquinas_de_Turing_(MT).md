...
- \( \Sigma \): Alfabeto de entrada (símbolos que pueden aparecer en la cadena de entrada).
- \( \Gamma \): Alfabeto de la cinta (\( \Sigma \subseteq \Gamma \)), incluye los símbolos de entrada y un símbolo especial \( B \) (blanco).
...
- \( B \in \Gamma \): Símbolo blanco (espacio vacío en la cinta).
...
1. **Cinta**:
   ...
   - Cada celda puede contener un símbolo del alfabeto \( \Gamma \).
...
3. **Estados**:
   ...
   - Según el estado actual y el símbolo leído, la función de transición \( \delta(q, X) = (q', Y, D) \) determina:
     - El nuevo estado (\( q' \)).
     - El símbolo que se escribe (\( Y \)).
     - La dirección del movimiento (\( D = L, R\)).

4. **Aceptación**:
   - Una cadena es aceptada si, después de una secuencia finita de transiciones, la máquina alcanza un estado final (\( q_f \in F\)).
